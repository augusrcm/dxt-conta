...
ORIGEN_PLANILLAS_REVERSO_PAGO = 'PLANILLAS_REVERSO_PAGO'
...
ORIGEN_LABELS = {
    ...
    ORIGEN_PLANILLAS_REVERSO_PAGO: 'Planillas - Reverso de pago',
}
...
def _usuario_puede_reabrir() -> bool:
    try:
        return int(session.get('rol_id')) in ROLES_REAPERTURA
    except (TypeError, ValueError):
        return False


def _es_reverso_pago_planilla(asiento: dict[str, Any]) -> bool:
    atributos = asiento.get('atributos') or {}
    return (
        _upper_clean(asiento.get('modulo_origen')) == 'PLANILLAS'
        and _clean(asiento.get('tabla_origen')) == 'contabilidad.pago'
        and bool(asiento.get('origen_id'))
        and _clean(atributos.get('accion')) == 'reverso_pago_planilla'
    )
...
def _obtener_asiento_basico(asiento_id: int) -> dict[str, Any] | None:
    asiento = execute_query_one(
        ...
        SELECT
            ...
            a.creado_en,
            a.actualizado_en,
            COALESCE(a.estado_uso, FALSE) AS estado_uso,
            a.unidad_negocio_id,
            ...
            CASE
                ...
                WHEN a.modulo_origen = 'PLANILLAS'
                     AND a.tabla_origen = 'contabilidad.pago'
                     AND COALESCE(a.atributos->>'accion', '') = 'reverso_pago_planilla' THEN 'PLANILLAS_REVERSO_PAGO'
                ELSE 'MANUAL'
            END AS origen,
            CASE
                ...
                WHEN a.modulo_origen = 'PLANILLAS'
                     AND a.tabla_origen = 'contabilidad.pago'
                     AND COALESCE(a.atributos->>'accion', '') = 'reverso_pago_planilla' THEN 'REVERSO_PAGO_PLANILLA'
                ELSE 'MANUAL'
            ...
            CASE
                ...
                WHEN a.modulo_origen = 'PLANILLAS'
                     AND a.tabla_origen = 'contabilidad.pago'
                     AND COALESCE(a.atributos->>'accion', '') = 'reverso_pago_planilla'
                     AND a.estado = 'BORRADOR' THEN TRUE
                ELSE FALSE
            END AS puede_editar,
            CASE
                WHEN a.modulo_origen = 'PLANILLAS'
                     AND a.tabla_origen = 'contabilidad.pago'
                     AND COALESCE(a.atributos->>'accion', '') = 'reverso_pago_planilla' THEN TRUE
                ELSE FALSE
            END AS es_reverso_pago_planilla,
            CASE WHEN COALESCE(a.atributos->>'reapertura_modo', '') <> '' THEN TRUE ELSE FALSE END AS reapertura_restringida,
            CASE
                WHEN COALESCE(a.modulo_origen, 'CONTABILIDAD') = 'CONTABILIDAD'
                     AND a.tabla_origen IS NULL
                     AND a.origen_id IS NULL
                     AND a.estado = 'CONFIRMADO'
                     AND COALESCE(a.estado_uso, FALSE) = FALSE THEN TRUE
                WHEN a.modulo_origen = 'PLANILLAS'
                     AND a.tabla_origen = 'contabilidad.pago'
                     AND COALESCE(a.atributos->>'accion', '') = 'reverso_pago_planilla'
                     AND a.estado = 'CONFIRMADO'
    ...
    if asiento and not _usuario_puede_reabrir():
        asiento['puede_reabrir_borrador'] = False
        if (asiento.get('atributos') or {}).get('reapertura_modo'):
            asiento['puede_editar'] = False
    return asiento
...
def _cuenta_bancaria_controlada(cuenta_codigo: str) -> bool:
    cuenta_codigo = _upper_clean(cuenta_codigo)
    ...
    row = execute_query_one(
        ...
        WHERE cuenta_contable_codigo = %s
          AND activo = TRUE
        LIMIT 1
        """,
        (cuenta_codigo,)
...
def _obtener_cuenta_bancaria_activa(
    cuenta_codigo: str,
    auxiliar_id: int | None,
    unidad_negocio_id: int | None,
    moneda_codigo: str,
) -> dict[str, Any] | None:
    if not cuenta_codigo or not auxiliar_id or not unidad_negocio_id or not moneda_codigo:
        return None
    ...
    return execute_query_one(
        ...
        WHERE cb.activo = TRUE
          AND cb.cuenta_contable_codigo = %s
          AND cb.auxiliar_id = %s
          AND cb.unidad_negocio_id = %s
          AND cb.moneda_codigo = %s
        LIMIT 1
        """,
        (cuenta_codigo, auxiliar_id, unidad_negocio_id, moneda_codigo)
    )



def _existen_cuentas_bancarias_para_unidad(
    cuenta_codigo: str,
    unidad_negocio_id: int | None,
    moneda_codigo: str,
) -> bool:
    if not cuenta_codigo or not unidad_negocio_id or not moneda_codigo:
        return False
    ...
    row = execute_query_one(
        ...
        LIMIT 1
        """,
        (cuenta_codigo, unidad_negocio_id, moneda_codigo)
    )
    return bool(row)
...
def _validar_detalles(
    ...
    for idx, item in enumerate(detalles, start=1):
        ...
        cuenta_bancaria_controlada = _cuenta_bancaria_controlada(cuenta_codigo)
        if cuenta_bancaria_controlada:
            if not _existen_cuentas_bancarias_para_unidad(
                cuenta_codigo,
                unidad_negocio_id,
                moneda_codigo,
            ):
                return None, (
                    f'La cuenta {cuenta_codigo} está vinculada a cuentas bancarias, '
                    f'pero no existe una cuenta bancaria activa en moneda {moneda_codigo} '
                    'para la unidad de negocio seleccionada.'
                )
            if not auxiliar_id:
                return None, f'La cuenta bancaria {cuenta_codigo} requiere seleccionar una cuenta bancaria registrada en la línea {idx}.'
            cuenta_bancaria = _obtener_cuenta_bancaria_activa(
                cuenta_codigo,
                auxiliar_id,
                unidad_negocio_id,
                moneda_codigo,
            ...
            if not cuenta_bancaria:
                return None, (
                    f'El auxiliar de la línea {idx} no corresponde a una cuenta bancaria activa '
                    f'en moneda {moneda_codigo} de la unidad de negocio seleccionada '
                    f'para la cuenta {cuenta_codigo}.'
                )
        elif bool(cuenta['requiere_auxiliar']) and not auxiliar_id:
...
def _validar_payload(data: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
    ...
    detalles_normalizados, error = _validar_detalles(
        detalles,
        glosa,
        unidad_negocio['id'],
        moneda_codigo,
...
def _crear_asiento_manual(payload: dict[str, Any], idempotency_key: str | None = None) -> int:
    with DatabaseManager() as db:
        cursor = db.conn.cursor(cursor_factory=RealDictCursor)

        key = _clean(idempotency_key)[:100] if idempotency_key else None
        if key:
            # Serializa solicitudes concurrentes provenientes de la misma pantalla.
            # La verificacion posterior evita crear dos borradores ante doble click/reintento.
            cursor.execute(
            ...
            cursor.execute(
                ...
                WHERE modulo_origen = %s
                  AND COALESCE(atributos->>'idempotency_key', '') = %s
                ORDER BY id
                LIMIT 1
                """,
                (MODULO_MANUAL, key),
        ...
        atributos_insert = dict(payload['atributos'])
        if key:
            atributos_insert['idempotency_key'] = key
        cursor.execute(
            """
            ...
            (
                ...
                MODULO_MANUAL,
                ESTADO_BORRADOR,
                json.dumps(atributos_insert, ensure_ascii=False),
            )
        )
        row = cursor.fetchone()
...
def _actualizar_fecha_reverso_planilla(asiento: dict[str, Any], fecha_nueva: date) -> None:
    atributos = dict(asiento.get('atributos') or {})
    atributos['ultima_actualizacion_por'] = _usuario_actual()
    atributos['ultima_actualizacion_en'] = datetime.now().isoformat(timespec='seconds')
    atributos['ultima_correccion_fecha'] = {
        'fecha_anterior': asiento.get('fecha').isoformat() if asiento.get('fecha') else None,
    ...
    with DatabaseManager() as db:
        ...
        cursor.execute(
            ...
            WHERE id = %s
              AND estado = %s
              AND modulo_origen = 'PLANILLAS'
              AND tabla_origen = 'contabilidad.pago'
              AND origen_id = %s
              AND COALESCE(atributos->>'accion', '') = 'reverso_pago_planilla'
            """,
            (
                fecha_nueva,
                json.dumps(atributos, ensure_ascii=False),
                asiento['id'],
                ESTADO_BORRADOR,
                asiento.get('origen_id'),
            ),
        )
        if cursor.rowcount != 1:
            raise ValueError('El reverso de planilla ya no está disponible para corregir su fecha.')


def _marcar_estado_manual(asiento: dict[str, Any], nuevo_estado: str) -> tuple[bool, str | None]:
    es_reverso_planilla = _es_reverso_pago_planilla(asiento)
    if not bool(asiento.get('es_manual')) and not es_reverso_planilla:
        return False, 'Este comprobante es administrado por otro módulo y se mantiene en solo lectura.'

    atributos = dict(asiento.get('atributos') or {})
    if atributos.get('reapertura_modo') and not _usuario_puede_reabrir():
        return False, 'No tiene permisos para confirmar un comprobante reabierto.'
    usuario = _clean(session.get('nombre')) or _clean(session.get('correo')) or f"USER-{session.get('user_id', 'NA')}"
    ...
    if nuevo_estado == ESTADO_CONFIRMADO:
        ...
        modo_reapertura = atributos.pop('reapertura_modo', None)
        if modo_reapertura:
            atributos['ultima_reapertura_modo'] = modo_reapertura
...
def _gestion_permite_reapertura(fecha_asiento: date | None) -> tuple[bool, str | None]:
    if not fecha_asiento:
        return False, 'El comprobante no tiene una fecha válida para validar la gestión.'

    gestion = int(fecha_asiento.year)
    row = execute_query_one(
        """
    ...
    # Si la gestión no existe en la tabla de control, no bloqueamos comprobantes
    # manuales para mantener compatibilidad con instalaciones previas del sistema.
    if not row:
        return True, None

    if _upper_clean(row.get('estado')) != 'ABIERTA':
        return False, f'No se puede volver a borrador porque la gestión {gestion} no está abierta.'

    return True, None


def _dependencias_operativas_asiento(asiento_id: int) -> list[str]:
    """Devuelve vínculos operativos que impiden editar un asiento confirmado.

    Se excluye documento_asiento porque es un respaldo documental del propio
    comprobante y no una operación que determine su contenido contable.
    """
    row = execute_query_one(
        """
        SELECT ARRAY_REMOVE(ARRAY[
            CASE WHEN EXISTS (SELECT 1 FROM contabilidad.pago WHERE asiento_id = %s) THEN 'Pagos' END,
            CASE WHEN EXISTS (SELECT 1 FROM contabilidad.cobro WHERE asiento_id = %s) THEN 'Cobros' END,
            CASE WHEN EXISTS (SELECT 1 FROM contabilidad.compra WHERE asiento_id = %s) THEN 'Compras' END,
            CASE WHEN EXISTS (SELECT 1 FROM contabilidad.venta WHERE asiento_id = %s) THEN 'Ventas' END,
            CASE WHEN EXISTS (SELECT 1 FROM contabilidad.movimiento_tesoreria WHERE asiento_id = %s) THEN 'Tesorería' END,
            CASE WHEN EXISTS (
                SELECT 1 FROM contabilidad.documento_por_cobrar
                WHERE asiento_registro_id = %s OR asiento_anulacion_id = %s
            ) THEN 'Documentos por cobrar' END,
            CASE WHEN EXISTS (
                SELECT 1 FROM contabilidad.planilla_periodo
                WHERE asiento_devengamiento_id = %s OR asiento_anulacion_id = %s
            ) THEN 'Planillas' END,
            CASE WHEN EXISTS (
                SELECT 1 FROM contabilidad.planilla_pago_aplicacion
                WHERE asiento_reversion_id = %s
            ) THEN 'Reversión de pagos de planilla' END,
            CASE WHEN EXISTS (
                SELECT 1 FROM contabilidad.planilla_prestamo
                WHERE asiento_otorgamiento_id = %s OR asiento_anulacion_id = %s
            ) THEN 'Préstamos de planilla' END,
            CASE WHEN EXISTS (
                SELECT 1 FROM contabilidad.planilla_prestamo_aplicacion
                WHERE asiento_id = %s
            ) THEN 'Aplicaciones de préstamos' END
        ], NULL) AS dependencias
        """,
        (
            asiento_id, asiento_id, asiento_id, asiento_id, asiento_id,
            asiento_id, asiento_id, asiento_id, asiento_id, asiento_id,
            asiento_id, asiento_id, asiento_id,
        ),
    )
    return list((row or {}).get('dependencias') or [])


def _snapshot_asiento_para_auditoria(asiento: dict[str, Any]) -> dict[str, Any]:
    detalles = _obtener_detalles_asiento(int(asiento['id']))
    return {
        'cabecera': {
            'fecha': asiento.get('fecha').isoformat() if asiento.get('fecha') else None,
            'moneda_codigo': asiento.get('moneda_codigo'),
            'tipo_cambio': str(asiento.get('tipo_cambio')) if asiento.get('tipo_cambio') is not None else None,
            'glosa': asiento.get('glosa'),
            'referencia': asiento.get('referencia'),
            'unidad_negocio_id': asiento.get('unidad_negocio_id'),
        ...
        'detalle': [
            {
                ...
                'monto_moneda': str(item.get('monto_moneda') or 0),
...
def _validar_reapertura_reverso_planilla(asiento: dict[str, Any]) -> tuple[bool, str | None]:
    if asiento.get('estado') != ESTADO_CONFIRMADO:
        return False, 'Solo se pueden volver a borrador comprobantes en estado CONFIRMADO.'
    if bool(asiento.get('estado_uso')):
        return False, 'Este comprobante ya fue utilizado por otro proceso operativo y no puede volver a borrador.'

    row = execute_query_one(
        """
        SELECT
            p.estado::text AS pago_estado,
            COUNT(ppa.id) AS total_aplicaciones,
            COUNT(ppa.id) FILTER (WHERE ppa.estado <> 'ANULADO') AS aplicaciones_no_anuladas
        FROM contabilidad.pago p
        LEFT JOIN contabilidad.planilla_pago_aplicacion ppa
          ON ppa.pago_id = p.id
         AND ppa.asiento_reversion_id = %s
        WHERE p.id = %s
        GROUP BY p.estado
        """,
        (asiento['id'], asiento.get('origen_id')),
    )
    if not row:
        return False, 'No se encontró el pago de planilla relacionado con este reverso.'
    if _upper_clean(row.get('pago_estado')) != 'ANULADO':
        return False, 'El pago relacionado debe permanecer ANULADO para corregir la fecha de su reverso.'
    if int(row.get('total_aplicaciones') or 0) < 1:
        return False, 'No se encontró la aplicación de planilla que vincula este asiento de reversión.'
    if int(row.get('aplicaciones_no_anuladas') or 0) > 0:
        return False, 'Existe una aplicación de planilla aún vigente; no es seguro reabrir este reverso.'

    return _gestion_permite_reapertura(asiento.get('fecha'))


def _validar_reapertura_borrador(asiento: dict[str, Any]) -> tuple[bool, str | None]:
    if not _usuario_puede_reabrir():
        return False, 'No tiene permisos para volver comprobantes confirmados a borrador.'

    if _es_reverso_pago_planilla(asiento):
        return _validar_reapertura_reverso_planilla(asiento)

    if not bool(asiento.get('es_manual')):
        return False, 'Solo los comprobantes manuales y los reversos de pago de planilla controlados pueden volver a borrador.'

    if asiento.get('estado') != ESTADO_CONFIRMADO:
        return False, 'Solo se pueden volver a borrador comprobantes en estado CONFIRMADO.'

    if asiento.get('tabla_origen') or asiento.get('origen_id'):
        return False, 'Este comprobante tiene una operación de origen relacionada y no puede volver a borrador desde este módulo.'

    if bool(asiento.get('estado_uso')):
        return False, 'Este comprobante ya fue utilizado por otro proceso operativo y no puede volver a borrador.'

    dependencias = _dependencias_operativas_asiento(int(asiento['id']))
    if dependencias:
        return False, (
            'El comprobante está relacionado con otro proceso operativo y no puede volver a borrador. '
            f'Vínculo detectado: {", ".join(dependencias)}.'
        )

    return _gestion_permite_reapertura(asiento.get('fecha'))


def _reabrir_asiento_manual_borrador(asiento: dict[str, Any], motivo: str) -> tuple[bool, str | None]:
    motivo = _clean(motivo)
    if len(motivo) < 10:
        return False, 'Debe registrar un motivo claro de al menos 10 caracteres.'

    ok, error = _validar_reapertura_borrador(asiento)
    if not ok:
        return False, error

    atributos = dict(asiento.get('atributos') or {})
    historial = atributos.get('reaperturas_borrador')
    ...
    snapshot = _snapshot_asiento_para_auditoria(asiento)

    historial.append({
        'tipo': 'VOLVER_A_BORRADOR',
        'asiento_id': asiento.get('id'),
        'estado_anterior': asiento.get('estado'),
        'estado_nuevo': ESTADO_BORRADOR,
        'fecha_asiento': asiento.get('fecha').isoformat() if asiento.get('fecha') else None,
        'motivo': motivo,
        'usuario_id': usuario_id,
        'usuario': usuario_nombre,
        'ejecutado_en': ahora,
        'snapshot_confirmado': snapshot,
    })

    atributos['reaperturas_borrador'] = historial
    atributos['ultimo_estado_anterior'] = asiento.get('estado')
    atributos['ultimo_cambio_estado'] = {
    ...
    atributos['reabierto_borrador_por'] = usuario_nombre
    atributos['reabierto_borrador_en'] = ahora
    atributos['reapertura_modo'] = 'FECHA_CONTROLADA' if _es_reverso_pago_planilla(asiento) else 'COMPLETA'
    ...
    with DatabaseManager() as db:
        ...
        if _es_reverso_pago_planilla(asiento):
            cursor.execute(
                ...
                WHERE id = %s
                  ...
                  AND origen_id = %s
                  AND COALESCE(atributos->>'accion', '') = 'reverso_pago_planilla'
                  AND COALESCE(estado_uso, FALSE) = FALSE
                """,
                (
                    ESTADO_BORRADOR,
                    json.dumps(atributos, ensure_ascii=False),
        ...
        if cursor.rowcount != 1:
            return False, 'El comprobante cambió de estado o ya no está disponible para volver a borrador.'
...
def data():
    ...
    elif origen == 'PLANILLAS_REVERSO_PAGO':
        where.append(
            "a.modulo_origen = 'PLANILLAS' AND a.tabla_origen = 'contabilidad.pago' "
            "AND COALESCE(a.atributos->>'accion', '') = 'reverso_pago_planilla'"
        )
    ...
    if not _usuario_puede_reabrir():
        for row in rows:
            row['puede_reabrir_borrador'] = False
            if bool(row.get('reapertura_restringida')):
                row['puede_editar'] = False
...
def buscar_auxiliares():
    q = _clean(request.args.get('q'))
    q_like = f'%{q}%'
    cuenta_codigo = _upper_clean(request.args.get('cuenta_codigo'))
    unidad_negocio_id = _parse_int_or_none(request.args.get('unidad_negocio_id'))
    moneda_codigo = _upper_clean(request.args.get('moneda_codigo'))

    if cuenta_codigo and _cuenta_bancaria_controlada(cuenta_codigo):
        params: list[Any] = [cuenta_codigo]
        unidad_filter = ''
        moneda_filter = ''

        if unidad_negocio_id:
            unidad_filter = ' AND cb.unidad_negocio_id = %s'
            params.append(unidad_negocio_id)

        if moneda_codigo:
            moneda_filter = ' AND cb.moneda_codigo = %s'
            params.append(moneda_codigo)

        params.extend([q, q_like, q_like, q_like, q_like])

        rows = execute_query(
            f"""
            SELECT
                ax.id,
                ax.tipo::text AS tipo,
                ax.nombre,
                COALESCE(ax.codigo_externo, '') AS codigo_externo,
                COALESCE(ax.nit_ci, '') AS nit_ci,
                cb.nombre_banco,
                cb.numero_cuenta,
                cb.titular
            FROM contabilidad.cuenta_bancaria cb
            INNER JOIN contabilidad.auxiliar ax
                ON ax.id = cb.auxiliar_id
            WHERE cb.activo = TRUE
              AND ax.activo = TRUE
              AND cb.cuenta_contable_codigo = %s
              {unidad_filter}
              ...
              AND (
                    %s = ''
                    OR ax.nombre ILIKE %s
                    OR COALESCE(ax.codigo_externo, '') ILIKE %s
                    OR COALESCE(ax.nit_ci, '') ILIKE %s
                    OR cb.numero_cuenta ILIKE %s
                  )
            ORDER BY cb.nombre_banco ASC, cb.numero_cuenta ASC, ax.nombre ASC
            LIMIT 30
            """,
            tuple(params),
            fetchall=True
        ...
        for row in rows:
            descriptor = [
                row.get('nombre_banco') or 'Banco',
                row.get('numero_cuenta') or '',
                row.get('titular') or row.get('nombre') or '',
                'BANCO',
            ]
            results.append({
                'id': row['id'],
                'text': ' | '.join([item for item in descriptor if _clean(item)]),
                'nombre': row['nombre'],
                'tipo': row['tipo'],
...
def crear():
    ...
    try:
        idempotency_key = _clean(data.get('idempotency_key'))
        asiento_id = _crear_asiento_manual(payload, idempotency_key)
        return jsonify({
            'ok': True,
...
def actualizar(asiento_id: int):
    ...
    if not bool(asiento.get('puede_editar')):
        return _make_error('El comprobante no está disponible para edición.')
    if (asiento.get('atributos') or {}).get('reapertura_modo') and not _usuario_puede_reabrir():
        return _make_error('No tiene permisos para editar un comprobante reabierto.', 403)

    data = request.get_json() or {}
    if _es_reverso_pago_planilla(asiento):
        fecha_nueva = _parse_date(data.get('fecha'))
        if not fecha_nueva:
            return _make_error('La fecha del comprobante es obligatoria.')
        gestion_ok, gestion_error = _gestion_permite_reapertura(fecha_nueva)
        if not gestion_ok:
            return _make_error(gestion_error or 'La fecha indicada pertenece a una gestión cerrada.', 409)
        try:
            _actualizar_fecha_reverso_planilla(asiento, fecha_nueva)
            return jsonify({
                'ok': True,
                'msg': f'Fecha del reverso de planilla {asiento_id} actualizada correctamente.',
            })
        except ValueError as exc:
            return _make_error(str(exc), 409)
        except Exception as exc:
            return _make_error(f'No se pudo actualizar la fecha del comprobante: {exc}', 500)

    payload, error = _validar_payload(data)
    if error:
        return _make_error(error)

    gestion_ok, gestion_error = _gestion_permite_reapertura(payload.get('fecha'))
    if not gestion_ok:
        return _make_error(gestion_error or 'La fecha indicada pertenece a una gestión cerrada.', 409)
...
@comprobantes_bp.route('/reabrir-borrador/<int:asiento_id>', methods=['POST'])
@login_required
def reabrir_borrador(asiento_id: int):
    asiento = _obtener_asiento_basico(asiento_id)
    if not asiento:
        return _make_error('El comprobante solicitado no existe.', 404)

    data = request.get_json(silent=True) or {}
    motivo = _clean(data.get('motivo'))

    ok, error = _reabrir_asiento_manual_borrador(asiento, motivo)
    if not ok:
        return _make_error(error or 'No se pudo volver a borrador el comprobante.')

    mensaje = (
        f'Comprobante {asiento_id} volvió a BORRADOR. Solo puede corregir la fecha y confirmar nuevamente.'
        if _es_reverso_pago_planilla(asiento)
        else f'Comprobante {asiento_id} volvió a BORRADOR. Ya puede editarlo y confirmar nuevamente.'
    )
