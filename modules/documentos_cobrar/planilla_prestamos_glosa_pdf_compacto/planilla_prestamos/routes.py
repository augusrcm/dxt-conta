...
def _validar_payload(data: dict[str, Any], gestion_activa: int) -> dict[str, Any]:
    ...
    moneda_codigo = _upper(data.get('moneda_codigo') or 'BOB')
    tipo_cambio = _money(data.get('tipo_cambio') or '1', 'Tipo de cambio')
    ...
    modalidad_registro = _upper(data.get('modalidad_registro') or 'DESEMBOLSO')
    ...
    medio_desembolso = _upper(data.get('medio_desembolso') or 'NO_APLICA')
...
def index():
    try:
        ...
        return render_template('planilla_prestamos_index.html', stats=stats, gestion_activa=date.today().year, puede_editar=_puede_editar(), error=str(exc))
    ...
    with DatabaseManager() as db:
        ...
        cobro_id = db.execute_insert(
            ...
            INSERT INTO contabilidad.cobro (
                fecha, cliente_auxiliar_id, medio_cobro, contra_cuenta_codigo,
...
def anular_borrador(prestamo_id: int):
    ...
    with DatabaseManager() as db:
        ...
        prestamo = _prestamo_by_id(db, prestamo_id)
        if prestamo['estado'] != 'BORRADOR':
            return _json_error('Solo se puede anular directamente un registro en BORRADOR.', 409)
        db.execute_update(
            """
            UPDATE contabilidad.planilla_prestamo
            SET estado = 'ANULADO', anulado_por = %s, anulado_en = CURRENT_TIMESTAMP,
                observacion = CONCAT(COALESCE(observacion,''), CASE WHEN COALESCE(observacion,'') = '' THEN '' ELSE E'\n' END, %s),
                actualizado_en = CURRENT_TIMESTAMP
            WHERE id = %s
            """,
            (_usuario_actual(), f'Anulado en borrador. Motivo: {motivo}', prestamo_id),
        )
    return _json_ok('Registro anulado.')
