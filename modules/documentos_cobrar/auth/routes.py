...
def _verify_turnstile_token(token: str) -> tuple[bool, str | None]:
    ...
    if not secret:
        return False, 'La configuración de seguridad no está completa. Falta la clave secreta de Turnstile.'
...
def login():
    ...
    if not identifier or not password:
        flash('Debes ingresar tu correo o CI/NIT y tu contraseña.', 'warning')
    ...
    if _is_locked(locked_key):
        flash('Tu acceso está temporalmente bloqueado por múltiples intentos. Espera unos minutos.', 'error')
        return render_template('login.html', **_login_context(identifier=identifier, captcha_checked=captcha_checked))
    ...
    try:
        ...
        current_app.logger.exception('Error consultando usuario de login.')
        flash(f'Error en el sistema: {exc}', 'error')
        return render_template('login.html', **_login_context(identifier=identifier, captcha_checked=captcha_checked))

    if not user or not _verify_password(password, user.get('password')):
        _register_attempt(locked_key)
        flash('Credenciales inválidas o sin permisos para Contabilidad.', 'error')
        return render_template('login.html', **_login_context(identifier=identifier, captcha_checked=captcha_checked))
