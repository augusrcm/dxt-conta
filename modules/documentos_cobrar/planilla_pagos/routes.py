...
def _cuenta_salida(db: DatabaseManager, medio: str, caja_id: int | None, cuenta_bancaria_id: int | None) -> dict[str, Any]:
    if medio == 'CAJA':
        rows = db.execute_query(
            """
            SELECT id, nombre, cuenta_contable_codigo
            FROM contabilidad.caja
        ...
        rows = db.execute_query(
            """
            SELECT id, nombre_banco || ' · ' || numero_cuenta AS nombre, cuenta_contable_codigo
            FROM contabilidad.cuenta_bancaria
            WHERE id = %s AND activo IS TRUE
            """,
            (cuenta_bancaria_id,)
...
def _crear_asiento_pago_planilla(db: DatabaseManager, pago_id: int, planilla: dict[str, Any], detalles: list[dict[str, Any]], cuenta_pasivo: str) -> int:
    ...
    db.execute_insert(
        ...
        """,
        (asiento_id, sec, salida['cuenta_contable_codigo'], f'Salida {pago["medio_pago"]} - {salida["nombre"]}'[:300], total, total, pago['referencia'], Json({'tipo': 'haber_salida_planilla'})),
        return_id=False
    )
    db.execute_insert(
        """
