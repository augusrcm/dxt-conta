...
# Registro de documentos por cobrar vigentes con asiento contable automatico.
# ============================================================
...
MODULO_ORIGEN = 'DOCUMENTOS_COBRAR_VIGENTE'
...
def _clasificar_documento(gestion_origen: int, gestion_actual: int) -> dict[str, str | bool]:
    ...
    return {
        'origen_documento': 'VIGENTE_MANUAL',
        'tratamiento_contable': 'CXC_NORMAL',
        'genera_asiento_registro': True,
        'etiqueta': 'Documento vigente',
    }
...
def _glosa_sugerida(payload: dict[str, Any]) -> str:
    ...
    partes = [
        f"Registro documento por cobrar {tipo} Nro. {payload['numero_documento']}",
...
def _movimientos_documento(db: DatabaseManager, documento_id: int) -> dict[str, Any] | None:
    ...
    glosa_inicial = documento.get('descripcion') or f"Registro documento por cobrar {tipo} Nro. {numero}, cliente {cliente}"
...
def _build_documento_pdf(documento: dict[str, Any], titulo: str) -> bytes:
    ...
    story = [Paragraph(titulo, title_style)]
    story.append(Paragraph(f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}", small))
    ...
    cuenta_contrap = f"{documento.get('cuenta_contrapartida_codigo') or ''} - {documento.get('cuenta_contrapartida_nombre') or ''}".strip(' -') or '-'
    unidad = f"{documento.get('unidad_codigo') or ''} - {documento.get('unidad_nombre') or ''}".strip(' -') or '-'

    def table(items):
        rows = []
        for label, value in items:
            rows.append([Paragraph(f'<b>{label}</b>', small), _pdf_paragraph(value, normal)])
        t = Table(rows, colWidths=[42 * mm, 136 * mm], hAlign='LEFT')
        t.setStyle(TableStyle([
            ('GRID', (0, 0), (-1, -1), 0.25, colors.HexColor('#dbe4ef')),
    ...
    t = Table(importes, colWidths=[30 * mm, 32 * mm, 38 * mm, 38 * mm, 38 * mm], hAlign='LEFT')
    t.setStyle(TableStyle([
        ('GRID', (0, 0), (-1, -1), 0.25, colors.HexColor('#dbe4ef')),
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#17406f')),
    ...
    story.append(table([
        ('Origen', ORIGENES_DOCUMENTO.get(documento.get('origen_documento'), documento.get('origen_documento'))),
        ('Tratamiento', TRATAMIENTOS_CONTABLES.get(documento.get('tratamiento_contable'), documento.get('tratamiento_contable'))),
        ('Cuenta de cartera/cobro', cuenta_cartera),
        ('Contrapartida registro', cuenta_contrap),
        ('Asiento de registro', documento.get('asiento_registro_id') or 'No genera asiento de registro'),
    ]))
...
@documentos_cobrar_bp.route('/help')
@login_required
@roles_required(ROLES_LECTURA)
def help():
    return render_template('documentos_cobrar_help.html')


@documentos_cobrar_bp.route('/data')
@login_required
@roles_required(ROLES_LECTURA)
def data():
    try:
        ...
        where = ["d.activo = TRUE", "d.origen_documento = 'VIGENTE_MANUAL'"]
        params: list[Any] = []
        ...
        with DatabaseManager() as db:
            total_rows = db.execute_query(
                "SELECT COUNT(*) AS total FROM contabilidad.documento_por_cobrar d WHERE d.activo = TRUE AND d.origen_documento = 'VIGENTE_MANUAL'"
            )
...
@documentos_cobrar_bp.route('/obtener/<int:documento_id>')
@login_required
@roles_required(ROLES_LECTURA)
...
@documentos_cobrar_bp.route('/crear', methods=['POST'])
@login_required
...
@documentos_cobrar_bp.route('/editar/<int:documento_id>', methods=['PUT'])
@login_required
@roles_required(ROLES_EDICION)
...
@documentos_cobrar_bp.route('/anular/<int:documento_id>', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
...
@documentos_cobrar_bp.route('/api/<int:documento_id>/movimientos')
@login_required
@roles_required(ROLES_LECTURA)
...
@documentos_cobrar_bp.route('/pdf/<int:documento_id>')
@login_required
@roles_required(ROLES_LECTURA)
def pdf(documento_id: int):
    ...
    if not row:
        return _json_error('Documento no encontrado.', 404)
    if row.get('origen_documento') != 'VIGENTE_MANUAL':
        return _json_error('Este PDF corresponde al módulo Documentos por Cobrar.', 400)

    asiento_id = row.get('asiento_registro_id')
    if asiento_id:
        return redirect(url_for('comprobantes.pdf', asiento_id=int(asiento_id)))

    pdf_bytes = _build_documento_pdf(row, 'Documento por Cobrar - Respaldo')
    filename = f"documento_por_cobrar_{int(documento_id):06d}.pdf"
    return Response(pdf_bytes, mimetype='application/pdf', headers={'Content-Disposition': f'inline; filename={filename}'})


@documentos_cobrar_bp.route('/api/clientes')
@login_required
@roles_required(ROLES_LECTURA)
def api_clientes():
...
@documentos_cobrar_bp.route('/api/cuentas')
@login_required
@roles_required(ROLES_LECTURA)
def api_cuentas():
    term = _clean(request.args.get('q'))
