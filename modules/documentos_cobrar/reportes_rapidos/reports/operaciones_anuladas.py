...
from modules.reportes_rapidos.reports.control_operativo_common import (
    ...
    validate_period_filters,
)


REPORT_ID = 'operaciones_anuladas'
TITLE = 'Operaciones anuladas'
DESCRIPTION = 'Control de anulaciones por periodo.'
WORKSHEET_TITLE = 'Operaciones anuladas'
FILE_SLUG = 'operaciones_anuladas'
...
HELP_TITLE = 'Operaciones anuladas'
HELP_INTRO = 'Permite revisar operaciones anuladas sin efecto operativo vigente.'
HELP_ITEMS = [
    'Muestra anulaciones de contabilidad, tesoreria, facturacion y cartera.',
...
def validate_filters(args):
    return validate_period_filters(args, default_grupo=DEFAULT_GRUPO, grupos=GRUPOS, fecha_label=FILTER_DATE_LABEL)
    ...
    sql = """
        WITH asiento_totales AS (
            ...
            SELECT
                ...
                COALESCE(NULLIF(a.glosa::text, ''), 'Asiento anulado')::text AS detalle,
                ...
                'Revisar respaldo'::text AS accion
            ...
            SELECT
                ...
                COALESCE(NULLIF(p.glosa::text, ''), 'Pago anulado')::text AS detalle,
                ...
                'Validar reversión'::text AS accion
            ...
            SELECT
                ...
                COALESCE(NULLIF(c.glosa::text, ''), 'Cobro anulado')::text AS detalle,
            ...
            SELECT
                ...
                CONCAT_WS(' · ', NULLIF(m.tipo_movimiento::text, ''), NULLIF(m.glosa::text, ''))::text AS detalle,
                ...
                'MEDIA'::text AS prioridad_codigo,
                'Media'::text AS prioridad,
                'Verificar reversión'::text AS accion
            ...
            SELECT
                ...
                COALESCE(NULLIF(f.cuf::text, ''), 'Factura anulada')::text AS detalle,
            ...
            WHERE UPPER(f.estado::text) IN ('ANULADA', 'ANULADO')
            ...
            SELECT
                ...
                COALESCE(d.anulado_en::date, d.fecha_documento::date) AS fecha,
                ...
                CONCAT_WS(' · ', NULLIF(d.tipo_documento::text, ''), NULLIF(d.descripcion::text, ''), NULLIF(d.motivo_anulacion::text, ''))::text AS detalle,
                ...
                'ALTA'::text AS prioridad_codigo,
                'Alta'::text AS prioridad,
                'Revisar cartera'::text AS accion
...
def build_payload(filtros, limit_rows=MAX_ROWS_SCREEN):
    ...
    filtros['descripcion'] = descripcion_periodo(filtros)
    return build_payload_common(
        ...
        rows,
        display_columns(),
        'No hay operaciones anuladas para los filtros seleccionados',
        include_money=True,
    )
