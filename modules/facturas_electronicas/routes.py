...
def _listar_facturas_operativas(cursor, filters):
    ...
    sql = f"""
        WITH reg AS (
            ...
            SELECT DISTINCT ON (da.origen_id)
                da.origen_id AS factura_electronica_id,
                da.asiento_id,
                COALESCE(a.estado::TEXT, '') AS asiento_estado,
                COALESCE(a.estado_uso, FALSE) AS asiento_estado_uso,
                COALESCE(a.modulo_origen, '') AS asiento_modulo_origen,
                COALESCE(a.tabla_origen, '') AS asiento_tabla_origen,
                a.origen_id AS asiento_origen_id
            ...
            ORDER BY da.origen_id, da.id DESC
        ), ventas_dir AS (
            SELECT
                factura_electronica_id,
                COUNT(*) AS ventas_vinculadas
            FROM contabilidad.venta
            WHERE factura_electronica_id IS NOT NULL
            GROUP BY factura_electronica_id
        ), dpc AS (
            SELECT
                factura_electronica_id,
                COUNT(*) AS documentos_cobrar_vinculados
        ...
        SELECT
            ...
            doc.asiento_id,
            COALESCE(doc.asiento_estado, '') AS asiento_estado,
            COALESCE(doc.asiento_estado_uso, FALSE) AS asiento_estado_uso,
            COALESCE(doc.asiento_modulo_origen, '') AS asiento_modulo_origen,
            COALESCE(doc.asiento_tabla_origen, '') AS asiento_tabla_origen,
            doc.asiento_origen_id,
            COALESCE(reg.cierre_manual_activo, FALSE) AS cierre_manual_activo,
            ...
            COALESCE(ventas_dir.ventas_vinculadas, 0) AS ventas_vinculadas,
            COALESCE(dpc.documentos_cobrar_vinculados, 0) AS documentos_cobrar_vinculados,
            CASE
        ...
        LEFT JOIN apps ON apps.factura_electronica_id = f.id
        LEFT JOIN doc ON doc.factura_electronica_id = f.id
        LEFT JOIN ventas_dir ON ventas_dir.factura_electronica_id = f.id
        LEFT JOIN dpc ON dpc.factura_electronica_id = f.id
        WHERE {' AND '.join(where)}
    ...
    for row in cursor.fetchall():
        items.append({
            ...
            'asiento_estado': row.get('asiento_estado') or '',
            'asiento_estado_uso': bool(row.get('asiento_estado_uso')),
            'asiento_modulo_origen': row.get('asiento_modulo_origen') or '',
            'asiento_tabla_origen': row.get('asiento_tabla_origen') or '',
            'asiento_origen_id': row.get('asiento_origen_id'),
            ...
            'ventas_vinculadas': int(row.get('ventas_vinculadas') or 0),
            'documentos_cobrar_vinculados': int(row.get('documentos_cobrar_vinculados') or 0),
        })
    return items
...
def _get_factura_bloqueos_deshacer(cursor, factura_id):
    cursor.execute(
        """
        WITH apps AS (
            SELECT
                COUNT(*) FILTER (
                    WHERE (fa.cobro_id IS NULL OR COALESCE(c.estado::TEXT, '') <> 'ANULADO')
                      AND (fa.venta_id IS NULL OR COALESCE(v.estado::TEXT, '') <> 'ANULADO')
                ) AS aplicaciones_activas,
                COUNT(*) FILTER (
                    WHERE fa.cobro_id IS NOT NULL
                      AND COALESCE(c.estado::TEXT, '') <> 'ANULADO'
                ) AS cobros_activos,
                COUNT(*) FILTER (
                    WHERE fa.venta_id IS NOT NULL
                      AND COALESCE(v.estado::TEXT, '') <> 'ANULADO'
                ) AS ventas_activas
            FROM contabilidad.factura_aplicacion fa
            LEFT JOIN contabilidad.cobro c ON c.id = fa.cobro_id
            LEFT JOIN contabilidad.venta v ON v.id = fa.venta_id
            ...
            SELECT
                COUNT(*) AS vinculos_asiento,
                MIN(asiento_id) AS asiento_id
            FROM contabilidad.documento_asiento
            WHERE tabla_origen = 'contabilidad.factura_electronica'
              AND origen_id = %s
        )
        SELECT
            COALESCE(apps.aplicaciones_activas, 0) AS aplicaciones_activas,
            COALESCE(apps.cobros_activos, 0) AS cobros_activos,
            COALESCE(apps.ventas_activas, 0) AS ventas_activas,
            COALESCE(reg.regularizaciones_activas, 0) AS regularizaciones_activas,
            COALESCE(reg.cierre_manual_activo, FALSE) AS cierre_manual_activo,
            COALESCE(docs.vinculos_asiento, 0) AS vinculos_asiento,
            docs.asiento_id
        FROM apps
        CROSS JOIN reg
        CROSS JOIN docs
...
def _get_factura_bloqueos_eliminar(cursor, factura_id):
    """Devuelve dependencias que impiden eliminar físicamente una factura cargada."""
    cursor.execute(
        """
        WITH apps AS (
            SELECT COUNT(*) AS aplicaciones_total
            FROM contabilidad.factura_aplicacion
            WHERE factura_electronica_id = %s
        ), reg AS (
            SELECT
                COUNT(*) AS regularizaciones_total,
                BOOL_OR(activo = TRUE AND tipo_regularizacion = 'CIERRE_MANUAL') AS cierre_manual_activo
            FROM contabilidad.factura_regularizacion
            WHERE factura_electronica_id = %s
        ), docs AS (
            SELECT COUNT(*) AS vinculos_asiento
            FROM contabilidad.documento_asiento
            WHERE tabla_origen = 'contabilidad.factura_electronica'
              AND origen_id = %s
        ), ventas AS (
            SELECT COUNT(*) AS ventas_total
            FROM contabilidad.venta
            WHERE factura_electronica_id = %s
        ), dpc AS (
            SELECT COUNT(*) AS documentos_cobrar_total
            FROM contabilidad.documento_por_cobrar
            WHERE factura_electronica_id = %s
        )
        SELECT
            COALESCE(apps.aplicaciones_total, 0) AS aplicaciones_total,
            COALESCE(reg.regularizaciones_total, 0) AS regularizaciones_total,
            COALESCE(reg.cierre_manual_activo, FALSE) AS cierre_manual_activo,
            COALESCE(docs.vinculos_asiento, 0) AS vinculos_asiento,
            COALESCE(ventas.ventas_total, 0) AS ventas_total,
            COALESCE(dpc.documentos_cobrar_total, 0) AS documentos_cobrar_total
        FROM apps
        CROSS JOIN reg
        CROSS JOIN docs
        CROSS JOIN ventas
        CROSS JOIN dpc
        """,
        (factura_id, factura_id, factura_id, factura_id, factura_id),
    )
    return cursor.fetchone() or {}


def _get_asiento_factura_for_update(cursor, asiento_id):
    cursor.execute(
        """
        SELECT
            id,
            fecha,
            COALESCE(estado::TEXT, '') AS estado,
            COALESCE(modulo_origen, '') AS modulo_origen,
            COALESCE(tabla_origen, '') AS tabla_origen,
            origen_id,
            COALESCE(estado_uso, FALSE) AS estado_uso,
            glosa,
            referencia,
            moneda_codigo,
            tipo_cambio,
            unidad_negocio_id,
            COALESCE(atributos, '{}'::jsonb) AS atributos
...
def _validar_gestion_abierta_por_fecha(cursor, fecha):
    if not fecha:
        raise ValueError('No se puede validar la gestión del asiento porque la fecha está vacía.')
    cursor.execute(
        """
        SELECT estado::TEXT AS estado
        FROM contabilidad.gestion_control
        WHERE gestion = EXTRACT(YEAR FROM %s::date)::int
        LIMIT 1
        """,
        (fecha,),
    )
    row = cursor.fetchone()
    if row and row.get('estado') != 'ABIERTA':
        raise ValueError('No se puede deshacer la contabilización porque la gestión del asiento no está abierta.')
...
@facturas_electronicas_bp.route('/api/<int:factura_id>/deshacer-contabilizacion', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
def api_deshacer_contabilizacion(factura_id):
    payload = request.get_json(silent=True) or {}
    try:
        motivo = _normalize_text(payload.get('motivo'), 'Motivo', 500, required=True)
        usuario = str(_usuario_actual())
        ...
        with DatabaseManager.get_cursor() as cursor:
            ...
            if not factura:
                raise ValueError('La factura seleccionada no existe.')
            if factura['estado'] == 'ANULADA':
                raise ValueError('La factura está anulada y no puede modificarse.')

            bloqueos = _get_factura_bloqueos_deshacer(cursor, factura_id)
            aplicaciones = int(bloqueos.get('aplicaciones_activas') or 0)
            cobros = int(bloqueos.get('cobros_activos') or 0)
            ventas = int(bloqueos.get('ventas_activas') or 0)
            regularizaciones = int(bloqueos.get('regularizaciones_activas') or 0)
            vinculos = int(bloqueos.get('vinculos_asiento') or 0)
            asiento_id = bloqueos.get('asiento_id') or factura.get('asiento_factura_id')

            if aplicaciones > 0 or cobros > 0 or ventas > 0:
                raise ValueError('No se puede deshacer la contabilización porque la factura ya tiene cobros o aplicaciones activas.')
            if regularizaciones > 0 or bloqueos.get('cierre_manual_activo'):
                raise ValueError('No se puede deshacer la contabilización porque la factura tiene regularizaciones o cierre de tesorería activo.')
            if vinculos != 1 or not asiento_id:
                raise ValueError('No se puede deshacer la contabilización porque el vínculo contable de la factura no es único o no existe.')

            asiento = _get_asiento_factura_for_update(cursor, asiento_id)
            if not asiento:
                raise ValueError('No se encontró el asiento contable asociado a la factura.')
            if asiento['estado'] != 'CONFIRMADO':
                raise ValueError('Solo se puede deshacer la contabilización de asientos confirmados.')
            if asiento['modulo_origen'] != 'TESORERIA' or asiento['tabla_origen'] != 'contabilidad.factura_electronica':
                raise ValueError('El asiento asociado no pertenece al flujo de facturas electrónicas de Tesorería.')
            if int(asiento['origen_id'] or 0) != int(factura_id):
                raise ValueError('El asiento asociado no corresponde a esta factura.')
            if asiento.get('estado_uso'):
                raise ValueError('No se puede deshacer la contabilización porque el asiento ya fue usado por otro proceso.')

            _validar_gestion_abierta_por_fecha(cursor, asiento['fecha'])

            auditoria = {
                'tipo': 'DESHACER_CONTABILIZACION_FACTURA_ELECTRONICA',
                'factura_id': int(factura_id),
                'asiento_anulado_id': int(asiento_id),
                'motivo': motivo,
                'usuario': usuario,
                'ejecutado_en': ejecutado_en,
                'estado_factura_anterior': factura.get('estado'),
                'fecha_contabilizacion_anterior': factura.get('fecha_contabilizacion').isoformat() if factura.get('fecha_contabilizacion') else None,
                'cuenta_cobrar_codigo_anterior': factura.get('cuenta_cobrar_codigo'),
                'cuenta_contra_codigo_anterior': factura.get('cuenta_contra_codigo'),
                'numero_factura': factura.get('numero_factura'),
                'nit_cliente': factura.get('nit_cliente'),
                'nombre_cliente': factura.get('nombre_cliente'),
                'importe_total': str(_decimal_from_db(factura.get('importe_total'))),
            }

            cursor.execute(
                """
                UPDATE contabilidad.asiento
                SET estado = 'ANULADO'::contabilidad.estado_generico_enum,
                    actualizado_en = CURRENT_TIMESTAMP,
                    atributos = jsonb_set(
                        COALESCE(atributos, '{}'::jsonb),
                        '{anulaciones_factura_electronica}',
                        COALESCE(
                            CASE
                                WHEN jsonb_typeof(COALESCE(atributos, '{}'::jsonb)->'anulaciones_factura_electronica') = 'array'
                                THEN COALESCE(atributos, '{}'::jsonb)->'anulaciones_factura_electronica'
                                ELSE '[]'::jsonb
                            END,
                            '[]'::jsonb
                        ) || %s::jsonb,
                ...
                WHERE id = %s
                """,
                (Json([auditoria]), asiento_id),
            )
            if cursor.rowcount != 1:
                raise ValueError('No se pudo anular el asiento asociado.')

            cursor.execute(
                """
                DELETE FROM contabilidad.documento_asiento
                WHERE modulo = 'TESORERIA'
                  AND tabla_origen = 'contabilidad.factura_electronica'
                  AND origen_id = %s
                  AND asiento_id = %s
                """,
                (factura_id, asiento_id),
            ...
            if cursor.rowcount != 1:
                raise ValueError('No se pudo desvincular el asiento de la factura de forma segura.')

            cursor.execute(
                """
                UPDATE contabilidad.factura_electronica
                SET cuenta_cobrar_codigo = NULL,
                    cuenta_contra_codigo = NULL,
                    fecha_contabilizacion = NULL,
                    actualizado_en = CURRENT_TIMESTAMP,
                    payload = jsonb_set(
                        COALESCE(payload, '{}'::jsonb) - 'contabilizacion',
                        '{deshacer_contabilizacion}',
                        COALESCE(
                            CASE
                                WHEN jsonb_typeof((COALESCE(payload, '{}'::jsonb) - 'contabilizacion')->'deshacer_contabilizacion') = 'array'
                                THEN (COALESCE(payload, '{}'::jsonb) - 'contabilizacion')->'deshacer_contabilizacion'
                                ELSE '[]'::jsonb
                            END,
                            '[]'::jsonb
                        ) || %s::jsonb,
                ...
                WHERE id = %s
                """,
                (Json([auditoria]), factura_id),
            )
            if cursor.rowcount != 1:
                raise ValueError('No se pudo actualizar la factura después de desvincular el asiento.')

            _recalcular_estado_factura(cursor, factura_id)

        return _json_success('Contabilización deshecha correctamente. El asiento anterior fue anulado y la factura quedó disponible para contabilizarse nuevamente.', factura_id=factura_id, asiento_id=asiento_id)
    except ValueError as exc:
        return _json_error(str(exc), status=400)
    except Exception as exc:
        return _json_error(f'No se pudo deshacer la contabilización. {exc}', status=500)


@facturas_electronicas_bp.route('/api/<int:factura_id>/eliminar-carga', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
def api_eliminar_carga_factura(factura_id):
    payload = request.get_json(silent=True) or {}
    try:
        motivo = _normalize_text(payload.get('motivo'), 'Motivo', 500, required=True)
        if len(motivo) < 8:
            raise ValueError('El motivo debe ser más descriptivo.')
        ...
        with DatabaseManager.get_cursor() as cursor:
            ...
            if factura.get('origen') != 'EXTERNO':
                raise ValueError('Solo se puede eliminar físicamente facturas cargadas desde importación externa.')

            bloqueos = _get_factura_bloqueos_eliminar(cursor, factura_id)
            detalles = []
            if int(bloqueos.get('vinculos_asiento') or 0) > 0:
                detalles.append('tiene asiento contable vinculado')
            if int(bloqueos.get('aplicaciones_total') or 0) > 0:
                detalles.append('tiene cobros, ventas o aplicaciones vinculadas')
            if int(bloqueos.get('regularizaciones_total') or 0) > 0:
                detalles.append('tiene regularizaciones o cierres de tesorería registrados')
            if int(bloqueos.get('ventas_total') or 0) > 0:
                detalles.append('tiene ventas vinculadas')
            if int(bloqueos.get('documentos_cobrar_total') or 0) > 0:
                detalles.append('tiene documentos por cobrar vinculados')
            if factura.get('asiento_factura_id'):
                detalles.append('mantiene referencia a un asiento contable')
            if factura.get('fecha_contabilizacion'):
                detalles.append('tiene fecha de contabilización')
            if factura.get('cuenta_cobrar_codigo') or factura.get('cuenta_contra_codigo'):
                detalles.append('tiene cuentas contables asignadas')

            if detalles:
                raise ValueError(
                    'No se puede eliminar físicamente esta factura porque ' + '; '.join(detalles) + '. '
                    'Primero debe deshacer la contabilización o revertir los movimientos relacionados, según corresponda.'
                )

            estado_actual = str(factura.get('estado') or '')
            if estado_actual not in {'RECIBIDA', 'DISPONIBLE', 'ANULADA'}:
                raise ValueError('Solo se puede eliminar una factura sin impacto operativo ni contable.')

            auditoria = {
                'tipo': 'ELIMINACION_FISICA_FACTURA_ELECTRONICA',
                'factura_id': int(factura_id),
                'motivo': motivo,
                'usuario': usuario,
                'ejecutado_en': ejecutado_en,
                ...
                'nit_cliente': factura.get('nit_cliente'),
                'nombre_cliente': factura.get('nombre_cliente'),
                'fecha_emision': factura.get('fecha_emision').isoformat() if factura.get('fecha_emision') else None,
                'importe_total': str(_decimal_from_db(factura.get('importe_total'))),
            }

            logger.warning(
                '[FACTURAS_ELECTRONICAS] Eliminacion fisica autorizada factura_id=%s numero=%s nit_cliente=%s usuario=%s motivo=%s',
                factura_id,
            ...
            cursor.execute(
                """
                DELETE FROM contabilidad.factura_electronica
                WHERE id = %s
                """,
                (factura_id,),
            )
            if cursor.rowcount != 1:
                raise ValueError('No se pudo eliminar la factura de forma segura.')

        return _json_success(
            'Factura cargada eliminada correctamente. Ya puede volver a importar o registrar la factura con los datos correctos.',
            factura_id=factura_id,
        )
    except ValueError as exc:
        return _json_error(str(exc), status=400)
    except Exception as exc:
        logger.exception('[FACTURAS_ELECTRONICAS] Error eliminando fisicamente factura_id=%s', factura_id)
        return _json_error(f'No se pudo eliminar la factura cargada. {exc}', status=500)
