...
# Reporte: Estado de Cartera por Cobrar
# ============================================================
...
def _fetch_compromisos(db, filtros):
    ...
    return db.execute_query(
        ...
        WHERE c.activo = TRUE
          ...
          AND d.fecha_vencimiento <= (%s::date + INTERVAL '100 years')
...
def _estado_item(item):
    dias = item.get('dias_vencido')
    aplicado = _to_decimal(item.get('aplicado'))
    saldo = _to_decimal(item.get('saldo'))
    if dias is None:
        situacion_codigo = 'SIN_VENCIMIENTO'
        situacion_label = 'Sin vencimiento'
        situacion_orden = 3
        badge = 'secondary'
    elif dias > 0:
        situacion_codigo = 'VENCIDA'
        situacion_label = 'Vencida'
        situacion_orden = 0
        badge = 'danger'
    elif dias == 0:
        situacion_codigo = 'VENCE_HOY'
        situacion_label = 'Vence hoy'
        situacion_orden = 1
        badge = 'warning'
    else:
        situacion_codigo = 'POR_VENCER'
        situacion_label = 'Por vencer'
        situacion_orden = 2
        badge = 'success'
    cobro_codigo = 'PARCIAL' if aplicado > 0 and saldo > 0 else 'SIN_COBRO'
    return {
        'situacion_codigo': situacion_codigo,
        'situacion_label': situacion_label,
        'situacion_orden': situacion_orden,
        'situacion_badge': badge,
        'cobro_codigo': cobro_codigo,
        'cobro_label': 'Parcial' if cobro_codigo == 'PARCIAL' else 'Sin cobro',
    }
...
def _estado_match(item, estado):
    ...
    if estado == 'PARCIAL':
        return meta['cobro_codigo'] == 'PARCIAL'
    return meta['situacion_codigo'] == estado


def _estado_principal(data):
    if data['vencidas'] > 0:
        return 'Vencida', 'danger', 0
    if data['vence_hoy'] > 0:
        return 'Vence hoy', 'warning', 1
    if data['parciales'] > 0:
        return 'Parcial', 'primary', 2
    if data['sin_vencimiento'] > 0:
        return 'Sin vencimiento', 'secondary', 3
    return 'Por vencer', 'success', 4


def _aggregate_general(items, filtros=None):
    estado = (filtros or {}).get('estado') or ''
    filtered = [item for item in items if _estado_match(item, estado)]
    grouped = {}
    for item in filtered:
        ...
        row['aplicado'] += item['aplicado']
        row['saldo'] += item['saldo']
        row['origenes'].add(item['origen_label'])
        if item.get('unidad_label'):
            row['unidades'].add(item['unidad_label'])
        if meta['situacion_codigo'] == 'VENCIDA':
            row['vencidas'] += 1
        elif meta['situacion_codigo'] == 'VENCE_HOY':
            row['vence_hoy'] += 1
        elif meta['situacion_codigo'] == 'POR_VENCER':
            row['por_vencer'] += 1
        elif meta['situacion_codigo'] == 'SIN_VENCIMIENTO':
            row['sin_vencimiento'] += 1
        if meta['cobro_codigo'] == 'PARCIAL':
            row['parciales'] += 1
        if item.get('dias_vencido') is not None and item['dias_vencido'] > row['mayor_antiguedad']:
            row['mayor_antiguedad'] = item['dias_vencido']

    rows = []
    for row in grouped.values():
        estado_label, estado_badge, estado_orden = _estado_principal(row)
        row['estado_label'] = estado_label
        row['estado_badge'] = estado_badge
        row['estado_orden'] = estado_orden
        row['origenes_label'] = ' · '.join(sorted(row['origenes']))
        row['unidades_label'] = ' · '.join(sorted(row['unidades']))
        row['total_label'] = _format_money(row['total'], row['moneda_codigo'])
        row['aplicado_label'] = _format_money(row['aplicado'], row['moneda_codigo'])
        row['saldo_label'] = _format_money(row['saldo'], row['moneda_codigo'])
        rows.append(row)

    rows.sort(key=lambda r: (
        r.get('estado_orden', 9),
        -r.get('vencidas', 0),
        -_to_decimal(r.get('saldo')),
...
def _build_summary(rows, items):
    ...
    clientes = set()
    vencidas = parciales = sin_vencimiento = vence_hoy = 0
    for item in items:
    ...
    for moneda, data in sorted(totals.items()):
        monedas.append({
            'moneda_codigo': moneda,
            'moneda_label': next((row.get('moneda_label') for row in rows if row.get('moneda_codigo') == moneda), moneda),
            'total_label': _format_money(data['total'], moneda),
            'aplicado_label': _format_money(data['aplicado'], moneda),
            'saldo_label': _format_money(data['saldo'], moneda),
        })
    saldo_label = '0.00'
    if len(monedas) == 1:
...
def _fetch_general(filtros):
    ...
    return _aggregate_general(items, filtros)
...
def _fetch_detalle(filtros, cliente_key, moneda_codigo):
    ...
    detalle = [
        ...
        if item['cliente_key'] == cliente_key
        and item['moneda_codigo'].upper() == moneda_codigo
        and _estado_match(item, estado)
    ]
    detalle.sort(key=lambda r: (
        _estado_item(r)['situacion_orden'],
        -(r.get('dias_vencido') or 0),
        r.get('fecha_vencimiento') or '9999-12-31',
        r.get('referencia') or '',
    ))
    for idx, row in enumerate(detalle, start=1):
        meta = _estado_item(row)
        row['nro'] = idx
        row['situacion_label'] = meta['situacion_label']
        row['situacion_badge'] = meta['situacion_badge']
        row['cobro_label'] = meta['cobro_label']
    return detalle
...
def _build_detail_summary(detalle):
    ...
    vencidas = parciales = sin_vencimiento = 0
    for item in detalle:
        total += item['total']
        ...
        if meta['situacion_codigo'] == 'VENCIDA':
            vencidas += 1
        if meta['situacion_codigo'] == 'SIN_VENCIMIENTO':
            sin_vencimiento += 1
        if meta['cobro_codigo'] == 'PARCIAL':
            parciales += 1
    return {
...
@estado_cuentas_cobrar_bp.route('/')
@login_required
...
def index():
    ...
    try:
        ...
        rows, items = [], []
        resumen = _build_summary(rows, items)
        error = f'No se pudo cargar el estado de cartera. {exc}'

    unidades, auxiliares, monedas = _fetch_catalogos()
    query_args = request.args.to_dict(flat=True)
    return render_template(
        'estado_cuentas_cobrar_index.html',
        filtros=filtros,
        unidades=unidades,
...
@estado_cuentas_cobrar_bp.route('/api/detalle')
@login_required
@roles_required(ROLES_LECTURA)
...
@estado_cuentas_cobrar_bp.route('/api/<int:auxiliar_id>/<moneda>/detalle')
@login_required
@roles_required(ROLES_LECTURA)
...
@estado_cuentas_cobrar_bp.route('/pdf')
@estado_cuentas_cobrar_bp.route('/pdf/general')
@login_required
@roles_required(ROLES_LECTURA)
def pdf_general():
    try:
        ...
        pdf_bytes = _build_pdf(
            title='Estado de Cartera por Cobrar',
            subtitle=f"{_periodo_label(filtros)} · Origen: {ORIGENES.get(filtros['origen'], 'Todos')} · Estado: {ESTADOS.get(filtros['estado'], 'Todos')}",
            columns=[
                {'label': 'Estado'}, {'label': 'Cliente'}, {'label': 'Mon.'}, {'label': 'Reg.'},
                {'label': 'Venc.'}, {'label': 'Parc.'}, {'label': 'Sin venc.'}, {'label': 'Orígenes'},
                {'label': 'Total'}, {'label': 'Cobrado'}, {'label': 'Pendiente'},
            ],
            rows=pdf_rows,
            col_widths=[20, 45, 13, 10, 11, 11, 17, 37, 24, 24, 24],
            pagesize=landscape(A4),
        )
        filename = f"estado_cartera_cobrar_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
...
@estado_cuentas_cobrar_bp.route('/detalle/pdf')
@login_required
@roles_required(ROLES_LECTURA)
def pdf_detalle_unificado():
    try:
        ...
        pdf_bytes = _build_pdf(
            title='Detalle de Estado de Cartera',
            subtitle=f"{cliente} · {resumen.get('moneda_label') or resumen['moneda_codigo']} · {_periodo_label(filtros)}",
            columns=[
                {'label': 'Estado'}, {'label': 'Origen'}, {'label': 'Referencia'}, {'label': 'Venc.'},
                {'label': 'Total'}, {'label': 'Cobrado'}, {'label': 'Pendiente'}, {'label': 'Cobro'},
            ],
            rows=pdf_rows,
            col_widths=[25, 31, 30, 21, 26, 26, 26, 25],
            pagesize=portrait(A4),
        )
        filename = f"estado_cartera_detalle_{_filename_safe(cliente)}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
        return Response(pdf_bytes, mimetype='application/pdf', headers={'Content-Disposition': f'inline; filename={filename}'})
    except Exception as exc:
        return Response(f'No se pudo generar el PDF del detalle. {exc}', status=500, mimetype='text/plain')


@estado_cuentas_cobrar_bp.route('/<int:auxiliar_id>/<moneda>/pdf')
@login_required
@roles_required(ROLES_LECTURA)
def pdf_auxiliar(auxiliar_id, moneda):
    try:
        ...
        for row in detalle:
            pdf_rows.append([
                row['situacion_label'], row['origen_label'], row['referencia'], row['fecha_vencimiento_label'],
                row['total_label'], row['aplicado_label'], row['saldo_label'], row['cobro_label'],
            ])
        cliente = resumen['cliente_nombre'] or 'Cliente'
        pdf_bytes = _build_pdf(
            title='Detalle de Estado de Cartera',
...
@estado_cuentas_cobrar_bp.route('/help')
@login_required
@roles_required(ROLES_LECTURA)
def help():
    return render_template('estado_cuentas_cobrar_help.html')
