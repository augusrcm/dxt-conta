...
def _fecha_contable_devengamiento(data: dict[str, Any], planilla: dict[str, Any]) -> date:
    ...
    if not valor:
        return planilla['fecha_planilla']
    fecha = _parse_date(valor, 'Fecha contable')
    if fecha is None:
        return planilla['fecha_planilla']
    gestion = int(planilla['gestion'])
...
def _codigo_planilla(db: DatabaseManager, gestion: int, mes: int) -> str:
    prefijo = f'PL-SU-{gestion}{mes:02d}'
...
def _catalogos(db: DatabaseManager) -> dict[str, Any]:
    ...
    conceptos = db.execute_query(
        ...
        WHERE activo = TRUE
          AND tipo_planilla IN ('PLANTA','AMBAS')
    ...
    personas = db.execute_query(
        ...
        WHERE pp.tipo_persona = 'PLANTA' AND pp.estado = 'ACTIVO'
...
def _conceptos_predefinidos(db: DatabaseManager) -> list[int]:
    ...
    defaults = db.execute_query(
        ...
        WHERE activo = TRUE
          AND tipo_planilla IN ('PLANTA','AMBAS')
          AND codigo IN ('HORAS_EXTRA','BONO_PRODUCCION','OTROS_INGRESOS','DESCUENTO_ATRASOS','DESCUENTO_JUDICIAL','OTROS_DESCUENTOS','APORTE_PATRONAL_ADICIONAL')
        ORDER BY
          CASE codigo
            WHEN 'HORAS_EXTRA' THEN 1
            WHEN 'BONO_PRODUCCION' THEN 2
            WHEN 'OTROS_INGRESOS' THEN 3
            WHEN 'DESCUENTO_ATRASOS' THEN 4
            WHEN 'DESCUENTO_JUDICIAL' THEN 5
            WHEN 'OTROS_DESCUENTOS' THEN 6
            WHEN 'APORTE_PATRONAL_ADICIONAL' THEN 7
            ELSE 99
          END
        """
    )
...
def _obtener_planilla(db: DatabaseManager, planilla_id: int) -> dict[str, Any] | None:
    rows = db.execute_query(
        ...
        WHERE pp.id = %s AND pp.tipo_planilla = 'PLANTA'
...
def _recalcular_detalle(db: DatabaseManager, detalle_id: int):
    base_row = db.execute_query(
        """
        SELECT COALESCE(haber_basico, monto_base, 0) AS haber_basico
        FROM contabilidad.planilla_detalle
        WHERE id = %s
        """,
        (detalle_id,)
    ...
    haber_basico = Decimal(str(base_row['haber_basico'] or 0)).quantize(CUANTIA)
    ingresos_adicionales = Decimal(str(row['ingresos_adicionales'] or 0)).quantize(CUANTIA)
    ingresos = haber_basico + ingresos_adicionales
    ...
    db.execute_update(
        ...
        WHERE id = %s
        """,
        (haber_basico, ingresos_adicionales, ingresos, descuentos, retenciones, aportes, liquido, liquido, detalle_id)
    )
...
def _build_planilla_pdf_story(planilla: dict[str, Any], conceptos: list[dict[str, Any]], detalles: list[dict[str, Any]]):
    ...
    headers = ['#', 'CI', 'Funcionario', 'Cargo', 'Días', 'Horas', 'Haber básico']
    for c in conceptos:
    ...
    for d in detalles:
        ...
        row = [
            ...
            _p(d.get('cargo_referencia') or d.get('ocupacion_referencia') or '', styles['DXTBody']),
            _p(_money_pdf(d.get('dias_trabajados')), styles['DXTBodyRight']),
            _p(_money_pdf(d.get('horas_pagadas')), styles['DXTBodyRight']),
            _p(_money_pdf(d.get('haber_basico') or d.get('monto_base')), styles['DXTBodyRight']),
        ]
        for c in conceptos:
...
@planilla_sueldos_planta_bp.route('/')
@login_required
...
def index():
    ...
    return render_template(
        'planilla_sueldos_planta_index.html',
        stats=stats,
        error=error,
...
@planilla_sueldos_planta_bp.route('/ayuda')
@login_required
@roles_required(ROLES_LECTURA)
def help():
    return render_template('planilla_sueldos_planta_help.html')


@planilla_sueldos_planta_bp.route('/opciones')
@login_required
@roles_required(ROLES_LECTURA)
def opciones():
    with DatabaseManager() as db:
...
@planilla_sueldos_planta_bp.route('/listar')
@login_required
@roles_required(ROLES_LECTURA)
def listar():
    ...
    filtros = ["pp.tipo_planilla = 'PLANTA'"]
    params: list[Any] = []
...
@planilla_sueldos_planta_bp.route('/generar', methods=['POST'])
@login_required
...
def generar():
    ...
    try:
        ...
        glosa = _limit_text(data.get('glosa') or f'Planilla de sueldos planta - {_mes_nombre(mes)} {gestion}', 'Glosa', 500, required=True)
        ...
        if not concepto_ids:
            raise ValueError('Seleccione al menos un concepto para generar la planilla.')
        ...
        with DatabaseManager() as db:
            ...
            if existentes:
                return _json_error(f'Ya existe una planilla de sueldos para ese mes: {existentes[0]["codigo"]}.')

            sin_unidad = db.execute_query(
                """
                SELECT nombre_completo, ci_nit
                FROM contabilidad.planilla_persona
                WHERE tipo_persona = 'PLANTA'
                  AND estado = 'ACTIVO'
            ...
            if sin_unidad:
                ...
                return _json_error('Existen personas de planta activas sin unidad de negocio asignada: ' + nombres)

            sin_basico = db.execute_query(
                """
                SELECT nombre_completo, ci_nit
                FROM contabilidad.planilla_persona
                WHERE tipo_persona = 'PLANTA'
                  AND estado = 'ACTIVO'
                  AND COALESCE(haber_basico_referencia, 0) <= 0
                ORDER BY nombre_completo
                LIMIT 8
                """
            )
            if sin_basico:
                nombres = ', '.join([f"{r['nombre_completo']} ({r['ci_nit']})" for r in sin_basico])
                return _json_error('Existen personas de planta activas sin haber básico asignado: ' + nombres)
            ...
            if not personas:
                return _json_error('No existen personas de planta activas para generar la planilla.')
            ...
            for cid in concepto_ids:
                if cid not in conceptos_by_id:
                    raise ValueError('Uno de los conceptos seleccionados no está activo o no aplica a planta.')
                conceptos_ordenados.append(conceptos_by_id[cid])

            parametros = _parametros_gestion(db, gestion)
            dias_mes = Decimal(str(monthrange(gestion, mes)[1])).quantize(CUANTIA)
            horas_mes = _horas_mes_referenciales(gestion, mes, Decimal(str(parametros.get('jornada_semanal_horas') or 48)))
            ...
            planilla_id = db.execute_insert(
                ...
                ) VALUES (%s, 'PLANTA', %s, %s, NULL, %s, %s, %s, 'BORRADOR', %s, %s, %s, CURRENT_TIMESTAMP)
                RETURNING id
                """,
            ...
            for idx, persona in enumerate(personas, start=1):
                ...
                detalle_id = db.execute_insert(
                    ...
                    INSERT INTO contabilidad.planilla_detalle (
                        ...
                        fecha_nacimiento, nacionalidad, sexo, ocupacion_referencia,
                        haber_basico, horas_pagadas,
                        tipo_respaldo, cantidad, precio_unitario, dias_trabajados, monto_base, estado,
                        observacion, atributos, creado_en
                    ) VALUES (%s, %s, %s, 'PLANTA', %s, %s, %s, %s, %s,
                              %s, %s, %s, %s, %s, %s, %s,
                              %s, %s, 'NO_APLICA', 1, 0, %s, %s, 'PENDIENTE', NULL, %s, CURRENT_TIMESTAMP)
                    RETURNING id
                    """,
                    (
                        ...
                        persona.get('fecha_nacimiento'), persona.get('nacionalidad'), persona.get('sexo'),
                        persona.get('ocupacion_referencia') or persona.get('cargo_referencia'),
                        Decimal(str(persona.get('haber_basico_referencia') or 0)).quantize(CUANTIA),
                        horas_mes,
                        dias_mes,
                        Decimal(str(persona.get('haber_basico_referencia') or 0)).quantize(CUANTIA),
                        Json(atributos)
                ...
                conceptos_estructurales = []
                if parametros.get('aplica_bono_antiguedad'):
                    anios = _anios_servicio(persona.get('fecha_ingreso_referencia'), gestion, mes)
                    porc_bono = _porcentaje_bono_antiguedad(anios)
                    base_bono = (Decimal(str(parametros.get('salario_minimo_nacional') or 0)) * Decimal(str(parametros.get('base_bono_smn_multiplicador') or 1))).quantize(CUANTIA)
                    monto_bono = (base_bono * porc_bono / Decimal('100')).quantize(CUANTIA, rounding=ROUND_HALF_UP)
                    if monto_bono > 0:
                        conceptos_estructurales.append({
                            'codigo': 'BONO_ANTIGUEDAD', 'nombre': 'Bono de antigüedad', 'tipo_concepto': 'INGRESO', 'impacto_liquido': 'SUMA',
                            'monto': monto_bono, 'porcentaje': porc_bono, 'justificativo': f'Cálculo asistido: {anios} año(s), {porc_bono}% sobre Bs {base_bono}.',
                            'atributos': {'origen': 'CALCULO_ASISTIDO', 'anios_servicio': anios, 'base_bono': str(base_bono)}
                        })

                haber = Decimal(str(persona.get('haber_basico_referencia') or 0)).quantize(CUANTIA)
                # AFP/Gestora se calcula sobre total ganado estructural inicial: haber básico + bono antigüedad.
                if parametros.get('aplica_afp_laboral'):
                    total_base_afp = haber + sum((c['monto'] for c in conceptos_estructurales if c['impacto_liquido'] == 'SUMA'), Decimal('0.00'))
                    porc_afp = Decimal(str(parametros.get('porcentaje_afp_laboral') or 0))
                    monto_afp = (total_base_afp * porc_afp / Decimal('100')).quantize(CUANTIA, rounding=ROUND_HALF_UP)
                    if monto_afp > 0:
                        conceptos_estructurales.append({
                            'codigo': 'AFP_LABORAL', 'nombre': 'AFP / Gestora laboral', 'tipo_concepto': 'RETENCION', 'impacto_liquido': 'RESTA',
                            'monto': monto_afp, 'porcentaje': porc_afp, 'justificativo': f'Cálculo asistido: {porc_afp}% sobre total ganado base Bs {total_base_afp}.',
                            'atributos': {'origen': 'CALCULO_ASISTIDO', 'base_calculo': str(total_base_afp)}
                        })

                if monto_cuotas > 0:
                    conceptos_estructurales.append({
                        'codigo': 'ANTICIPO_PRESTAMO', 'nombre': 'Anticipos / préstamos programados', 'tipo_concepto': 'DESCUENTO', 'impacto_liquido': 'RESTA',
                        'monto': monto_cuotas, 'porcentaje': None, 'justificativo': 'Descuento automático por anticipos/préstamos programados para la planilla.',
                        'atributos': {'origen': 'ANTICIPO_PRESTAMO_PROGRAMADO', 'prestamo_cuotas': [
                ...
                secuencia_actual = 1
                for estructural in conceptos_estructurales:
                    db.execute_insert(
                        """
                        ...
                        ) VALUES (%s, %s, NULL, %s, %s, %s, %s, %s, %s, %s, NULL, NULL, %s, NULL, %s, CURRENT_TIMESTAMP)
                        """,
                        (
                            planilla_id, detalle_id, secuencia_actual, estructural['codigo'], estructural['nombre'], estructural['tipo_concepto'],
                            estructural['impacto_liquido'], estructural['monto'], estructural.get('porcentaje'), estructural.get('justificativo'), Json(estructural.get('atributos') or {}),
                        ),
                        return_id=True,
                    )
                    secuencia_actual += 1
                ...
                for sec, concepto in enumerate(conceptos_ordenados, start=secuencia_actual):
                    ...
                    justificativo = None
                    atributos_concepto = {
                        'requiere_justificativo': bool(concepto.get('requiere_justificativo')),
                        'metodo_calculo': concepto.get('metodo_calculo'),
                        'origen': 'CONCEPTO_BASE',
                    }
                    ...
                    db.execute_insert(
                        ...
                        (
                            ...
                            justificativo, concepto.get('observacion'), Json(atributos_concepto)
                        ),
                        return_id=False
                    )
        ...
        return _json_ok('Planilla de sueldos generada en borrador.', redirect=url_for('planilla_sueldos_planta.detalle', planilla_id=planilla_id))
    except ValueError as exc:
...
@planilla_sueldos_planta_bp.route('/<int:planilla_id>')
@login_required
@roles_required(ROLES_LECTURA)
def detalle(planilla_id: int):
    with DatabaseManager() as db:
        ...
        if not planilla:
            return redirect(url_for('planilla_sueldos_planta.index'))
        conceptos = _conceptos_planilla(db, planilla_id)
        detalles = _detalle_planilla(db, planilla_id)
    return render_template(
        'planilla_sueldos_planta_detalle.html',
        planilla=planilla,
        conceptos=conceptos,
        detalles=detalles,
        meses=dict(MESES),
...
@planilla_sueldos_planta_bp.route('/<int:planilla_id>/pdf')
@login_required
@roles_required(ROLES_LECTURA)
...
@planilla_sueldos_planta_bp.route('/api/<int:planilla_id>/fila/<int:detalle_id>', methods=['GET'])
@login_required
@roles_required(ROLES_LECTURA)
def fila(planilla_id: int, detalle_id: int):
    with DatabaseManager() as db:
        detalle_rows = db.execute_query(
            ...
            WHERE pd.id = %s AND pd.planilla_periodo_id = %s AND pp.tipo_planilla = 'PLANTA'
            """,
...
@planilla_sueldos_planta_bp.route('/api/<int:planilla_id>/fila/<int:detalle_id>', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
def guardar_fila(planilla_id: int, detalle_id: int):
    ...
    try:
        ...
        dias_pagados = _decimal(data.get('dias_pagados'), 'Días pagados', allow_zero=True)
        horas_pagadas = _decimal(data.get('horas_pagadas'), 'Horas pagadas', allow_zero=True)
        haber_basico = _decimal(data.get('haber_basico'), 'Haber básico', allow_zero=True)
        observacion = _limit_text(data.get('observacion'), 'Observación', 800)
        ...
        with DatabaseManager() as db:
            ...
            db.execute_update(
                ...
                WHERE id = %s
                """,
                (dias_pagados, horas_pagadas, haber_basico, haber_basico, observacion, detalle_id)
            )
            _recalcular_detalle(db, detalle_id)
            _recalcular_planilla(db, planilla_id)
...
@planilla_sueldos_planta_bp.route('/api/<int:planilla_id>/fila/<int:detalle_id>/excluir', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
...
@planilla_sueldos_planta_bp.route('/api/<int:planilla_id>/consolidar', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
...
@planilla_sueldos_planta_bp.route('/api/<int:planilla_id>/revertir', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
...
@planilla_sueldos_planta_bp.route('/api/<int:planilla_id>/eliminar', methods=['POST'])
@login_required
@roles_required(ROLES_EDICION)
def eliminar(planilla_id: int):
    ...
    try:
        ...
        with DatabaseManager() as db:
            ...
            eliminados = db.execute_delete("DELETE FROM contabilidad.planilla_periodo WHERE id = %s AND tipo_planilla = 'PLANTA'", (planilla_id,))
            if eliminados != 1:
                return _json_error('La planilla no pudo ser eliminada porque no cumple las condiciones operativas.', 409)
        return _json_ok('Planilla eliminada correctamente.', redirect=url_for('planilla_sueldos_planta.index'))
    except ValueError as exc:
        return _json_error(str(exc))
    except Exception as exc:
...
def _validar_parametros_contables(db: DatabaseManager, planilla_id: int, parametros: dict[str, Any]):
    _requiere_cuenta(parametros, 'cuenta_gasto_sueldos_codigo', 'Gasto sueldos / salarios')
    _requiere_cuenta(parametros, 'cuenta_sueldos_por_pagar_codigo', 'Sueldos por pagar')
    rows = db.execute_query(
        """
...
def _insertar_asiento(db: DatabaseManager, fecha: date, unidad_id: int, moneda: str, tipo_cambio: Decimal,
    ...
    asiento_id = db.execute_insert(
        ...
        """,
        (fecha, unidad_id, moneda, tipo_cambio, glosa[:500], referencia[:150], planilla_id, Json({'origen': 'planilla_sueldos_planta', 'accion': accion, 'planilla_id': planilla_id}))
    )
    sec = 1
    for linea in lineas:
...
def _crear_asientos_devengamiento(db: DatabaseManager, planilla_id: int, planilla: dict[str, Any], parametros: dict[str, Any], justificativo: str, fecha_contable: date | None = None) -> list[int]:
    ...
    for u in unidades:
        ...
        glosa = f'Devengamiento {planilla["codigo"]} - {u["unidad_codigo"]}'
        referencia = planilla['codigo']
        lineas: list[dict[str, Any]] = []
        aportes = row_decimal(u, 'aportes')
        liquido = row_decimal(u, 'liquido')
        total_ganado = row_decimal(u, 'total_ganado')
        _agregar_linea(lineas, parametros['cuenta_gasto_sueldos_codigo'], debe=total_ganado, glosa=glosa, atributos={'tipo': 'debe_total_ganado'})
        if aportes > 0:
        ...
        _agregar_linea(lineas, parametros['cuenta_sueldos_por_pagar_codigo'], haber=liquido, glosa=glosa, atributos={'tipo': 'haber_liquido_pagable'})
        ...
        asiento_id = _insertar_asiento(
            ...
            glosa, referencia, 'devengamiento_planilla_planta', planilla_id, lineas, vincular_documento=(len(asientos) == 0))
...
def _crear_asientos_reversion(db: DatabaseManager, planilla_id: int, planilla: dict[str, Any], justificativo: str) -> dict[int, int]:
    rows = db.execute_query(
        ...
        WHERE a.estado = 'CONFIRMADO'
          ...
          AND a.origen_id = %s
          AND COALESCE(a.atributos->>'accion','') = 'devengamiento_planilla_planta'
        UNION
        SELECT DISTINCT a.*
        FROM contabilidad.asiento a
    ...
    for asiento in rows:
        ...
        reverso_id = _insertar_asiento(
            ...
            'reverso_devengamiento_planilla_planta', planilla_id, lineas, vincular_documento=False,
        )
